...
minimize_lbfgsb_gradient = 0*gpml_unwrap(X);
minimize_lbfgsb_X = X;

X0 = X;
lb = -Inf*ones(size(gpml_unwrap(X0)));
ub =  Inf*ones(size(gpml_unwrap(X0)));
...
X = lbfgsb( gpml_unwrap(X0), lb, ub, 'minimize_lbfgsb_objfun', ...
...
% Non-numerical elements are ignored. See also the reverse gpml_rewrap.m. 
function v = gpml_unwrap(s)
  v = [];   
  if isnumeric(s)
...
% should be empty. Non-numerical entries are just copied. See also gpml_unwrap.m.
function [s v] = gpml_rewrap(s, v)
  if isnumeric(s)
    if numel(v) < numel(s)
